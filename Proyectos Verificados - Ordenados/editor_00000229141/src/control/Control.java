...
public class Control {
    ...
    public int reemplazar(JFrame frame) {
        if (texto != null) {
            ...
            DlgReemplazar dlgReemplazar = new DlgReemplazar(frame, true, buscar, reemplazar, ConstantesGUI.REMPLAZAR, respuesta);
    ...
    public void contar(JFrame frame) throws IOException, FileNotFoundException {
        if (texto != null) {
            int numPal = 0;
            StreamTokenizer palabra = new StreamTokenizer(new StringReader(texto));
            while (palabra.nextToken() != StreamTokenizer.TT_EOF) {
                if (palabra.ttype == StreamTokenizer.TT_WORD) {
                    numPal++;
                }
            }
